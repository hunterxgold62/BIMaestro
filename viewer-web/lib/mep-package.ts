...
export async function openViewerPackage(url: string, fetchAsset?: () => Promise<Response>): Promise<ViewerPackage> {
  const response = fetchAsset ? await fetchAsset() : await fetch(url, { cache: 'no-store', referrerPolicy: 'no-referrer' });
