...
import { sectionContains } from './mep-section';
...
export class BuildingVisibility {
  ...
  sync() {
    for (const mesh of this.meshes) {
      ...
      entry.proxy.visible = isObjectVisible(mesh) && entry.proxy.geometry.getAttribute('position').count > 0;
    }
  }
  obscured(camera: THREE.Vector3, anchor: THREE.Vector3, planes: THREE.Plane[] = []) {
    ...
    this.ray.firstHitOnly = planes.length === 0;
    return this.ray.intersectObjects([...this.entries.values()].filter(entry => entry.proxy.visible).map(entry => entry.proxy), false).some(hit => sectionContains(planes, hit.point));
  }
  dispose() {
