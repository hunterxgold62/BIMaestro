...
import { markupQuaternion, reservationGeometry, type Markup } from './mep-markup';
import { reservationBounds } from './mep-reservation-volume';
import { isReservationHost } from './mep-host';
...
export function cutWall(source: THREE.BufferGeometry, world: THREE.Matrix4, marks: Markup[], elementId: number) {
  ...
  try {
    ...
    if (halfEdges.unmatchedEdges > 0) throw new Error('La géométrie complète de cet élément comporte des faces manquantes. La découpe ne peut pas être calculée.');
    for (const mark of marks) {
      ...
      const box = reservationGeometry(mark, .002);
      ...
      try {
        ...
        if (Array.from(geometry.getAttribute('position').array).some(n => !Number.isFinite(n))) throw new Error('Découpe impossible sur cet élément.');
...
export class WallCutouts {
  ...
  constructor(private meshes: THREE.Mesh[], private properties: Map<number, WebProperty>) {
    for (const mesh of meshes) {
      ...
      for (let i = 0; i < positions.count; i++) {
        ...
        if (!isReservationHost(this.properties.get(id))) continue;
  ...
  update(marks: Markup[]) {
    ...
    for (const [index, property] of this.properties) if (isReservationHost(property)) {
    ...
    try {
      for (const [id, reservations] of cuts) {
        ...
        try {
          ...
          if (!wall) throw new Error('Les faces de cet élément ne peuvent pas être réunies.');
          ...
          removed.add(id);
        } catch (error) { errors.push(error instanceof Error ? error.message : 'Découpe impossible sur cet élément.'); }
