...
// Rotate the camera AND its panned viewing target around a separate selected pivot.
// Panning and travelling remain free without changing which object we orbit.
export function orbitSelection(camera: THREE.Camera, target: THREE.Vector3, pivot: THREE.Vector3, dx: number, dy: number) {
  const offset = camera.position.clone().sub(pivot);
  if (offset.lengthSq() < 1e-8) return;
  const yaw = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 1, 0), -dx);
  const right = new THREE.Vector3(1, 0, 0).applyQuaternion(camera.quaternion).applyQuaternion(yaw).normalize();
  const view = camera.getWorldDirection(new THREE.Vector3());
  const elevation = Math.asin(THREE.MathUtils.clamp(view.y, -1, 1));
  const pitch = new THREE.Quaternion().setFromAxisAngle(right, THREE.MathUtils.clamp(elevation - dy, -1.48, 1.48) - elevation);
  const rotation = pitch.multiply(yaw);
  camera.position.copy(offset.applyQuaternion(rotation).add(pivot));
