...
import { markupQuaternion, reservationSize, type Markup } from './mep-markup';

export function reservationBounds(mark: Markup) {
  const depth = mark.depthCm / 30.48;
  const [width, height] = reservationSize(mark);
  const bounds = new THREE.Box3(
    new THREE.Vector3(-width / 60.96, -height / 60.96, -depth - .002),
    new THREE.Vector3(width / 60.96, height / 60.96, .002),
  );
