...
import { ArrowDown, ArrowUp, Box, ChevronRight, CircleGauge, Contrast, Eye, EyeOff, Footprints, Hand, Layers3, LoaderCircle, LockKeyhole, Pencil, Plane, RotateCcw, SunMedium, Undo2, Users, Waves } from 'lucide-react';
import * as THREE from 'three';
import { GLTFLoader } from 'three/examples/jsm/loaders/GLTFLoader.js';
import { reservationExchange, reservationsIfc, downloadExchange } from '@/lib/mep-exchange';
import { ElementSelection, zoomStep } from '@/lib/mep-selection';
...
import { BuildingVisibility, blocksView } from '@/lib/mep-visibility';
import { ModelStream, tileBounds, disposeModel } from '@/lib/mep-stream';
import { isWall } from '@/lib/mep-cutouts';
import { ModelPreparation } from '@/lib/mep-prepare';
import { AsyncWallCutouts } from '@/lib/mep-cutouts-async';
import { markupObject, disposeMarkups, type Markup } from '@/lib/mep-markup';
import { saveMarkup, readScenario } from '@/lib/mep-api';
...
import { applyScenario } from '@/lib/mep-engine';
import { resolveShare, shareTokenFromLocation, subscribeToScenario, updateSource, updateValve } from '@/lib/mep-api';
...
type ContextMenuState = { x: number; y: number; index: number; position: [number, number, number]; normal: [number, number, number] };
...
function flowStateLabel(value: number | string | undefined) {
  ...
  if (normalized === 2 || normalized === 'supplied') return 'alimenté';
...
function reliabilityLabel(value: number | string | undefined) {
  const normalized = typeof value === 'string' ? value.toLowerCase() : value;
  if (normalized === 0 || normalized === 'reliable') return 'Sens fiable';
...
export function MepViewer() {
  ...
  const valveState = JSON.stringify(scenario?.state.valves || {});
  const sourceState = JSON.stringify(scenario?.state.sources || {});
  const graph = useMemo(() => viewerPackage ? applyScenario(viewerPackage.replay.graph, JSON.parse(valveState), JSON.parse(sourceState)) : null, [viewerPackage, valveState, sourceState]);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const pkg = await openViewerPackage(access.packageUrl);
        if (disposed) { pkg.dispose(); return; }
        setResolved(access); setViewerPackage(pkg); setScenario(access.scenario);
  ...
  const visibleMarkups = useMemo(() => allMarkups.filter(mark => !mark.needsReview), [allMarkups]);
  useEffect(() => {
    const marks = visibleMarkups;
    markupsRef.current = marks;
    const group = markupGroupRef.current;
    if (group) { disposeMarkups(group); marks.forEach(mark => group.add(markupObject(mark))); }
  ...
  }, [visibleMarkups, viewerPackage, propertiesByKey]);
  useEffect(() => {
    if (!resolved) return;
    let disposed = false;
    const refresh = async () => {
  ...
  useEffect(() => {
    ...
    markupsRef.current.forEach(mark => markupGroup.add(markupObject(mark)));
    ...
    let pivotDestination: THREE.Vector3 | null = null;
    ...
    controls.target.set(0, 1.5, 0); controls.enableDamping = true; controls.dampingFactor = .14; controls.rotateSpeed = 1.15; controls.panSpeed = 1.2; controls.enableZoom = false; controls.screenSpacePanning = true;
    ...
    let draggingEdit = false, dragBase: Markup | null = null, previewMark: Markup | null = null;
    ...
    gizmo.addEventListener('objectChange', () => {
      ...
      setDraftMarkup({ ...dragBase, position: editRoot.position.toArray(), widthCm: dimensions(dragBase.widthCm * Math.abs(editRoot.scale.x)), heightCm: dimensions(dragBase.heightCm * Math.abs(editRoot.scale.y)) });
    });
    ...
    scene.add(new THREE.GridHelper(100, 100, '#91a2aa', '#b8c3c8'));
    ...
    let previousView: { position: THREE.Vector3; quaternion: THREE.Quaternion; target: THREE.Vector3 } | null = null;
    ...
    const updateMepState = (nextGraph: MepGraph | null, nextHiddenSystems: Set<string>) => {
      ...
      for (const animation of animatedFlowArrows) {
        ...
        animation.mesh.visible = !nextHiddenSystems.has(animation.systemKey) && (!stream || stream.complete(propertiesByKey.get(animation.elementKey)?.index ?? -1)) &&
          (state === 2 || state === 'supplied') && path?.hasCirculation === true;
    ...
    viewActionsRef.current = {
      frameSelected: () => {
        ...
        previousView = { position: camera.position.clone(), quaternion: camera.quaternion.clone(), target: controls.target.clone() };
        ...
        controls.target.copy(sphere.center); pivotDestination = null; controls.update();
      },
      isometric: () => { previousView = { position: camera.position.clone(), quaternion: camera.quaternion.clone(), target: controls.target.clone() }; frameModel(); },
      previous: () => { if (!previousView) return; camera.position.copy(previousView.position); camera.quaternion.copy(previousView.quaternion); controls.target.copy(previousView.target); controls.update(); previousView = null; },
      enterSpectator,
    ...
    const addDemo = () => {
      ...
      const pipe = (r: number, length: number, material: THREE.Material, p: [number, number, number], rotation: [number, number, number]) => {
        const mesh = new THREE.Mesh(new THREE.CylinderGeometry(r, r, length, 32), material); mesh.position.set(...p); mesh.rotation.set(...rotation); scene.add(mesh);
      };
      ...
      const valve = new THREE.Mesh(new THREE.IcosahedronGeometry(.48, 2), valveMaterial); valve.position.set(0, 2.7, 0); scene.add(valve); placeModelLights(); applyAppearance(); frameModel();
    };
    if (viewerPackage) {
      const resetGeometry = () => {
      ...
      const updateCuts = () => {
        ...
        const marks = markupsRef.current.filter(mark => !stream || stream.complete(propertiesByKey.get(mark.elementKey)?.index ?? -1));
      ...
      const addModel = async (model: THREE.Group) => {
        ...
        if (!viewerPackage.manifest.tiles) {
          modelSphere = new THREE.Box3().setFromObject(model).getBoundingSphere(new THREE.Sphere());
        ...
        for (const [pathOrdinal, { path, points }] of paths.entries()) {
          ...
          if (length > .02 && animatedFlowArrows.length < 1200) {
            ...
            for (let index = 0; index < count && animatedFlowArrows.length < 1200; index++) {
              ...
              arrow.visible = supplied && path.hasCirculation;
    ...
    let pointerStart: { x: number; y: number } | null = null; let looking = false; let suppressContextMenu = false;
    let previousFrameTime = performance.now();
    let pickedAnchor: Pick<ContextMenuState, 'position' | 'normal'> | null = null;
    const selectAt = (event: { clientX: number; clientY: number }) => {
      pickedAnchor = null;
      const selectable = pickables;
      ...
      for (const hit of raycaster.intersectObjects(selectable, false)) {
        ...
        if (relocatingRef.current) {
          setDraftMarkup(anchorMarkup({ ...relocatingRef.current, ...pickedAnchor }, property, resolved!.publication.revision));
    ...
    const onPointerDown = (event: PointerEvent) => {
      ...
      if (navigationModeRef.current === 'orbit' && event.button === 0 && selectedIndexRef.current === null) {
        const rect = renderer.domElement.getBoundingClientRect();
    ...
    const onPointerUp = (event: PointerEvent) => {
      ...
      const distance = pointerStart ? Math.hypot(event.clientX - pointerStart.x, event.clientY - pointerStart.y) : 0;
      looking = false; pointerStart = null;
      if (renderer.domElement.hasPointerCapture(event.pointerId)) renderer.domElement.releasePointerCapture(event.pointerId);
      if (event.button === 2 && distance >= 5) suppressContextMenu = true;
    ...
    const onKeyDown = (event: KeyboardEvent) => {
      if (!sceneReady) return;
      if (event.code === 'Escape') { setRelocating(null); setDraftMarkup(null); setContextMenu(null); selectedIndexRef.current = null; setSelectedIndex(null); }
      if (draftOpenRef.current || (event.target instanceof HTMLElement && event.target.closest('input,textarea,[contenteditable=true]'))) { pressed.clear(); return; }
    ...
    const onDoubleClick = (event: MouseEvent) => { if (draftRef.current || !sceneReady) return; const id = selectAt(event); if (id !== null) { selection.set(pickables.filter((m): m is THREE.Mesh => m instanceof THREE.Mesh), new Set([id])); viewActionsRef.current?.frameSelected(); } };
    ...
    const animate = () => {
      ...
      stream?.update(now);
      controls.enabled = sceneReady && !draggingEdit && navigationModeRef.current === 'orbit';
      ...
      if (lastSelection !== selectedIndexRef.current) {
        ...
        pivotDestination = lastSelection === null || selection.bounds.isEmpty() ? null : selection.bounds.getCenter(new THREE.Vector3());
      }
      selection.sync();
      if (!draggingEdit && previewMark !== draftRef.current) {
        previewMark = draftRef.current; disposeMarkups(editRoot); editRoot.scale.set(1, 1, 1);
        if (previewMark) {
          const preview = markupObject(previewMark); editRoot.position.copy(preview.position); editRoot.quaternion.copy(preview.quaternion);
          ...
          gizmo.attach(editRoot);
        } else gizmo.detach();
      }
      if (gizmo.mode !== editModeRef.current) gizmo.setMode(editModeRef.current);
      if (navigationModeRef.current === 'orbit' && pivotDestination) {
        controls.target.lerp(pivotDestination, 1 - Math.exp(-12 * delta));
        if (controls.target.distanceToSquared(pivotDestination) < .00001) pivotDestination = null;
      }
      ...
      for (const label of Array.from(markupLabelsRef.current?.children || [])) {
        ...
        const mark = markupsRef.current.find(item => item.id === element.dataset.markup);
        if (!mark) continue;
        const projected = new THREE.Vector3(...mark.position).project(camera);
        const visible = projected.z >= -1 && projected.z <= 1 && Math.abs(projected.x) < 1 && Math.abs(projected.y) < 1;
        if (visible && refreshVisibility) {
          const hidden = buildingVisibility?.obscured(camera.position, new THREE.Vector3(...mark.position));
          element.style.opacity = hidden ? '0.2' : '1';
        }
    ...
    return () => {
      ...
      cutoutsRef.current?.dispose(); cutoutsRef.current = null;
      disposeMarkups(markupGroup); markupGroupRef.current = null;
      ...
      renderer.domElement.removeEventListener('pointerdown', onPointerDown); renderer.domElement.removeEventListener('pointermove', onPointerMove); renderer.domElement.removeEventListener('pointerup', onPointerUp); renderer.domElement.removeEventListener('contextmenu', onContextMenu); renderer.domElement.removeEventListener('wheel', onWheel, true);
  ...
  const beginMarkup = (kind: Markup['kind']) => {
    ...
    setDraftMarkup({ id: crypto.randomUUID(), kind, elementKey: property.key, elementName: property.name, position: contextMenu.position, normal: contextMenu.normal, text: '', widthCm: 60, heightCm: 40, depthCm: 30, modelRevision: resolved!.publication.revision });
    setContextMenu(null);
  };
  ...
  const exportReservations = (ifc: boolean) => {
    try {
      if (!viewerPackage || !resolved) return;
      const exchange = reservationExchange(visibleMarkups, viewerPackage.manifest, resolved.publication.id);
      if (!exchange.reservations.length) throw new Error('Aucune réservation confirmée à exporter.');
      downloadExchange(ifc ? reservationsIfc(exchange) : JSON.stringify(exchange, null, 2), ifc ? 'reservations.ifc' : 'reservations.bimaestro-reservations.json');
    } catch (caught) { setError(caught instanceof Error ? caught.message : 'Export impossible'); }
  };
  const draftIsWall = isWall(viewerPackage?.properties.find(property => property.key === draftMarkup?.elementKey));
  ...
  return <><main className="viewer-shell" inert={opening}>
    {draftMarkup && <section className="markup-editor" aria-label="Modifier l’annotation">
        ...
        <p>Glissez les poignées sur le mur. La découpe sera calculée après « Enregistrer ».</p>
        {draftMarkup && <form onSubmit={event => { event.preventDefault(); void persistMarkup(draftMarkup); }}>
          ...
          {draftMarkup.kind === 'reservation' && <><div className="markup-dimensions">{(['widthCm', 'heightCm', 'depthCm'] as const).map((key, index) => <label key={key}>{['Largeur (cm)', 'Hauteur (cm)', 'Profondeur (cm)'][index]}<input type="number" required min="1" max="1000" step="0.1" value={draftMarkup[key] || ''} onChange={event => setDraftMarkup({ ...draftMarkup, [key]: Number(event.target.value) })}/></label>)}</div><p>{draftIsWall ? 'La profondeur saisie est mesurée depuis la face sélectionnée vers l’intérieur du mur. Augmentez-la pour créer une ouverture traversante.' : 'Le volume est centré sur le point sélectionné. Seuls les murs peuvent être découpés.'}</p></>}
    ...
    <header className="topbar"><div className="brand-mark"><Waves size={19}/><span>BIMaestro</span><b>Maquette MEP</b></div><div className="model-title"><span>{resolved?.publication.name || 'Démonstration · Local technique'}</span><Badge variant="outline">Révision {resolved?.publication.revision || 'démo'}</Badge></div><div className={`access-badge ${isEditor ? 'editor' : 'reader'}`}>{isEditor ? <Pencil size={15}/> : <LockKeyhole size={15}/>}<span><small>MODE DU LIEN</small>{isEditor ? 'ÉDITION COLLABORATIVE' : 'CONSULTATION · LECTURE SEULE'}</span></div></header>
    <section className="viewport"><div ref={mountRef} className="three-mount"/><div className="view-toolbar"><Button size="sm" variant="secondary" onClick={() => viewActionsRef.current?.isometric()}><Box/> Isométrique</Button><Button size="sm" variant="secondary" disabled={selectedIndex === null} onClick={() => viewActionsRef.current?.frameSelected()}>Cadrer la sélection · F</Button><Button size="sm" variant="secondary" onClick={() => setMarkupListOpen(v => !v)}>Notes / réservations ({allMarkups.length})</Button><Button size="sm" variant="secondary" aria-label="Vue précédente" title="Revenir à la vue avant l’isométrique" onClick={() => viewActionsRef.current?.previous()}><Undo2/> Vue précédente</Button></div><div className="appearance-toolbar" aria-label="Réglages de rendu"><button className={realisticLighting ? 'active' : ''} onClick={() => setRealisticLighting((value) => !value)} title="Basculer entre le mode ombré du plugin et les couleurs uniformes"><SunMedium/><span>{realisticLighting ? 'Ombré' : 'Uniforme'}</span></button><button className={highContrast ? 'active' : ''} onClick={() => setHighContrast((value) => !value)} title="Renforcer les contours et le contraste"><Contrast/><span>Contraste</span></button><button className={modelOpacity < 1 ? 'active' : ''} onClick={cycleOpacity} title="Faire varier la transparence de la maquette"><Layers3/><span>{Math.round(modelOpacity * 100)} %</span></button></div><div className="navigation-switch" aria-label="Mode de navigation"><button className={navigationMode === 'orbit' ? 'active' : ''} onClick={() => changeNavigationMode('orbit')}><Hand size={15}/><span>Navigation web</span></button><button className={navigationMode === 'maquette' ? 'active' : ''} onClick={() => changeNavigationMode('maquette')}><Footprints size={15}/><span>Maquette MEP</span></button></div>{navigationMode === 'maquette' && <div className="flight-controls" aria-label="Commandes de déplacement vertical"><button className={flightMode ? 'active' : ''} onClick={() => viewActionsRef.current?.toggleFlight()} title={flightMode ? 'Revenir au mode marche' : 'Activer le vol libre'}><Plane/><span>{flightMode ? 'Quitter le vol' : 'Vol libre'}</span></button>{flightMode && <><button aria-label="Monter" title="Monter" onPointerDown={() => holdInput('Space', true)} onPointerUp={() => holdInput('Space', false)} onPointerCancel={() => holdInput('Space', false)} onPointerLeave={() => holdInput('Space', false)}><ArrowUp/><span>Monter</span></button><button aria-label="Descendre" title="Descendre" onPointerDown={() => holdInput('ShiftLeft', true)} onPointerUp={() => holdInput('ShiftLeft', false)} onPointerCancel={() => holdInput('ShiftLeft', false)} onPointerLeave={() => holdInput('ShiftLeft', false)}><ArrowDown/><span>Descendre</span></button></>}<button onClick={() => viewActionsRef.current?.enterSpectator()} title="Revenir au point de départ"><RotateCcw/><span>Réapparaître</span></button></div>}{contextMenu && isEditor && contextProperty && <div className="element-context-menu" style={{ left: contextMenu.x, top: contextMenu.y }} role="menu"><header><span>{contextProperty.name}</span><small>#{contextProperty.elementId}</small></header><button onClick={() => beginMarkup('note')}>Annoter ici</button><button onClick={() => beginMarkup('reservation')}>Créer une réservation ici</button>{contextMepElement && <>{contextValve && <button disabled={saving} onClick={() => void runScenarioEdit(contextMepElement, 'valve', !contextValve.isClosed)}>{contextValve.isClosed ? 'Ouvrir la vanne' : 'Fermer la vanne'}</button>}<button disabled={saving} className={contextSourceKind === 'inlet' ? 'active' : ''} onClick={() => void runScenarioEdit(contextMepElement, 'source', 'inlet')}>{contextSourceKind === 'inlet' ? '✓ Départ du réseau' : 'Définir comme départ'}</button><button disabled={saving} className={contextSourceKind === 'outlet' ? 'active' : ''} onClick={() => void runScenarioEdit(contextMepElement, 'source', 'outlet')}>{contextSourceKind === 'outlet' ? '✓ Retour du réseau' : 'Définir comme retour'}</button>{contextSourceKind && <button disabled={saving} className="danger" onClick={() => void runScenarioEdit(contextMepElement, 'source', 'none')}>Retirer {contextSourceKind === 'inlet' ? 'le départ' : 'le retour'}</button>}</>}<button onClick={() => setContextMenu(null)}>Fermer</button></div>}<div ref={markupLabelsRef} className="markup-labels">{visibleMarkups.map(mark => <button key={mark.id} data-markup={mark.id} className={`markup-label ${mark.kind}`} onClick={() => { setActiveMarkup(mark.id); setMarkupError(null); }} title={mark.text || 'Réservation'}>{mark.kind === 'note' ? '●' : '▣'} <span>{mark.kind === 'note' ? mark.text : `${mark.widthCm} × ${mark.heightCm} cm`}</span></button>)}</div>{activeMark && <div className="markup-card"><strong>{activeMark.kind === 'note' ? 'Annotation' : 'Réservation'}</strong><small>{activeMark.elementName}</small>{activeMark.needsReview && <p aria-live="polite">Élément modifié, absent ou ancien export : replacez ce point avant de réappliquer la découpe.</p>}<p>{activeMark.text}</p>{activeMark.kind === 'reservation' && <p>{activeMark.widthCm} × {activeMark.heightCm} cm · Profondeur {activeMark.depthCm} cm</p>}{markupError && <p role="alert">{markupError}</p>}{isEditor && <><Button size="sm" disabled={activeMark.needsReview} onClick={() => { setEditMode("translate"); setDraftMarkup({ ...activeMark }); setActiveMarkup(null); }}>Modifier / déplacer</Button><Button size="sm" onClick={() => { setRelocating(activeMark); setActiveMarkup(null); }}>Replacer sur une face</Button><Button size="sm" variant="outline" disabled={saving} onClick={() => void persistMarkup(activeMark, true)}>Supprimer</Button></>}<Button size="sm" variant="ghost" onClick={() => setActiveMarkup(null)}>Fermer</Button></div>}{relocating && <div className="relocation-hint">Cliquez sur la nouvelle face pour replacer « {relocating.text || relocating.elementName} ». Échap pour annuler.</div>}
    {markupListOpen && <aside className="markup-list"><strong>Notes et réservations</strong><div className="markup-actions"><Button size="sm" variant="outline" disabled={!visibleMarkups.some(m => m.kind === 'reservation')} onClick={() => exportReservations(true)}>Exporter IFC</Button><Button size="sm" variant="outline" disabled={!visibleMarkups.some(m => m.kind === 'reservation')} onClick={() => exportReservations(false)}>Exporter vers Revit</Button></div><p>Volumes de réservation, dans le repère interne du projet Revit. Les points à vérifier sont exclus.</p><Button size="sm" variant="ghost" onClick={() => setMarkupListOpen(false)}>Fermer</Button>{!allMarkups.length && <p>Aucune annotation enregistrée.</p>}{allMarkups.map(mark => <button key={mark.id} onClick={() => { setActiveMarkup(mark.id); setMarkupListOpen(false); }}><b>{mark.kind === 'note' ? 'Note' : 'Réservation'} · {mark.elementName}</b><span>{mark.needsReview ? 'Emplacement à vérifier — ancienne révision' : mark.text || mark.widthCm + ' × ' + mark.heightCm + ' cm'}</span></button>)}</aside>}
      ...
      {selectedProperty ? <>
        ...
        <article className="inspection-card">
          ...
          {selectedMepElement && <section className="selected-mep-information">
            ...
            <p>État : {selectedPath?.flowState === 2 && !selectedPath.hasCirculation ? 'sous pression, fluide stagnant' : flowStateLabel(selectedMepElement.flowState)}{selectedPath ? <><br/>Sens : {selectedPath.flowForward ? 'début → fin' : 'fin → début'}</> : null}</p>
