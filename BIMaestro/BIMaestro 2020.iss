; --------------------------------------------------------
; BIMaestro.iss — Installateur per‑user sans UAC, .addin 2020
...
; Dossier où sera généré le .addin pour Revit 2020
...
// -----------------------------------------------------------------
// Après copie, crée le manifest pour Revit 2020
// -----------------------------------------------------------------
procedure CurStepChanged(CurStep: TSetupStep);
var
  AddinsRoot, Dir2020, BinFolder, Xml: String;
begin
  ...
  begin
    ...
    // 3) Création du dossier 2020
    Dir2020 := AddinsRoot + '\2020';
    ForceDirectories(Dir2020);

    // 4) Génération du XML
    Xml := BuildXml(BinFolder);

    // 5) Sauvegarde du manifest .addin
    SaveStringToFile(Dir2020 + '\BIMaestro.addin', Xml, False);
  end;
end;
