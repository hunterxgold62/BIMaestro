...
namespace Modification
{
            ...
            try
            {
                ...
                RouteCandidate TryComputeAStar(double gridStepMm, int[] xySteps, double heuristicBias)
                {
                    RouteCandidate Process(RouteCandidate candidate)
                    {
                        ...
                        var processed = PostProcess(candidate.Points, obstaclesRaw, obstaclesForSnap, clearMm, snapTolMm, jogTolMm, minSegMm);
                    ...
                    var primary = Process(AStarMulti(start, end, obstaclesRaw, gridStepMm, zStepMm, clearMm, exemptMm, xySteps, heuristicBias));
                    if (primary != null) return primary;

                    if (!ReferenceEquals(obstaclesForSnap, obstaclesRaw) && obstaclesForSnap != null && obstaclesForSnap.Count > 0)
                        return Process(AStarMulti(start, end, obstaclesForSnap, gridStepMm, zStepMm, clearMm, exemptMm, xySteps, heuristicBias));
    ...
    .ToList();
        ...
        {
            var u = new BoundingBoxXYZ { Min = new XYZ(double.MaxValue, double.MaxValue, double.MaxValue),
