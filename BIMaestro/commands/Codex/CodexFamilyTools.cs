    ...
    internal static class CodexFamilyTools
    {
        ...
        internal static IEnumerable<JObject> Definitions()
        {
            ...
            yield return Tool("revit_family_api", "Recherche les types et signatures natifs réellement disponibles dans cette version Revit. type_name vide liste les types correspondant à member_name ; sinon nom CLR complet du type, member_name filtre ses membres. Pagination offset/limit. Utiliser avec le contrat de programmes avant de construire un appel inconnu.", new JObject {
                ...
                ["offset"] = new JObject { ["type"] = "integer", ["minimum"] = 0, ["maximum"] = 100000 }, ["limit"] = new JObject { ["type"] = "integer", ["minimum"] = 1, ["maximum"] = 100 } });
            yield return Tool("revit_run_family_program", "Exécute un programme d'opérations natives dans la famille OUVERTE, quelle que soit son origine. Lire revit_family_program_contract, revit_family_api et l'inventaire avant. program_json contient steps et outputs ; objets/collections/calculs/boucles/familles imbriquées. Tester d'abord avec validate_only=true (annulation intégrale), puis appliquer le même programme avec false. Pas de C# ni d'accès système, fichiers ou autres documents. Une opération annulable, aucun nouveau RFA parent. Permet une géométrie détaillée et des répétitions de modules complets sans les restrictions de formes des outils spécialisés. Revit peut refuser une géométrie/contrainte invalide ; rapport précis par étape.", new JObject {
                ["document_key"] = new JObject { ["type"] = "string", ["maxLength"] = 100 }, ["description"] = new JObject { ["type"] = "string", ["maxLength"] = 1000 },
                ["validate_only"] = new JObject { ["type"] = "boolean" }, ["program_json"] = new JObject { ["type"] = "string", ["maxLength"] = 200000 } });
        ...
        internal static object Capabilities(string version) => new {
            ...
            limitations = new[] { "Les limites de formes ci-dessous concernent les outils spécialisés ; revit_run_family_program expose une voie API générale pour les profils/solides/vides, modules imbriqués et réseaux natifs. Consulter le contrat et les signatures avant de refuser ou simplifier une demande.", "Extrusions selon X/Y/Z : profils rectangulaires ou polygonaux droits pilotés par leurs sommets ; barres arrays inclinables de 0 à 180 degrés.", "Les pièces cachées doivent rester géométriquement valides.", "Les paramètres existants compatibles sont réutilisés sans distinction de majuscules. Pour un paramètre partagé existant, fournir son GUID. Un conflit de type, de formule ou de portée non convertible exige un autre nom ; ne pas réessayer le même nom.",
                ...
                "Ces opérations ne sont pas prévues par le constructeur déclaratif spécialisé ; consulter revit_family_api et le moteur de programmes pour les construire avec les API natives disponibles, puis valider.",
