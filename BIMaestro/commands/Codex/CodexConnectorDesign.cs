    ...
    internal sealed class FamilyConnectorSpec
    {
        ...
        internal static List<FamilyConnectorSpec> Parse(JObject source, CodexParametricDesign design)
        {
            ...
            foreach (var token in CodexFamilyDesign.Items(source, "connectors", 0, 16))
            {
                ...
                if (item.Domain == "duct" && !new[] { "mechanical", "air_terminal", "duct_accessory", "duct_fitting" }.Contains(design.Metadata.Category) || item.Domain == "pipe" && !new[] { "mechanical", "plumbing", "pipe_accessory", "pipe_fitting" }.Contains(design.Metadata.Category) || item.Domain == "electrical" && !new[] { "electrical", "mechanical", "lighting" }.Contains(design.Metadata.Category)) throw new InvalidOperationException("Catégorie incompatible avec le connecteur.");
