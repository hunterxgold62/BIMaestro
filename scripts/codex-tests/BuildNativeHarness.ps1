param([ValidateSet('2023','2024')][string]$RevitVersion = '2023', [ValidatePattern('^[a-zA-Z0-9-]*$')][string]$RunName = '', [string]$FixtureFilter = '*.json', [switch]$FamilyEditOnly, [switch]$FamilyProgramOnly)
...
[string[]]$defines = @(if ($FamilyEditOnly) { '/define:FAMILY_EDIT_ONLY' }; if ($FamilyProgramOnly) { '/define:FAMILY_PROGRAM_ONLY' })
